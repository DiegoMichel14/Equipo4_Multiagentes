...
class Vehicle(ap.Agent):
    ...
    def setup(self):
        ...
        if self.direction == 1:
            self.position = (20, self.posx)
        else:
            self.position = (self.posx, 6)





   # Función auxiliar para verificar si hay algún agente en la posición de parada
    def is_agent_ahead_stopped(self):
        ...
        for agent in self.model.agents:
            if agent.position == stop_position and agent.stop:
                return True
        return False
    ...
    def action(self):
        ...
        # Agregar ceros a la secuencia de movimiento cuando el vehículo está detenido
        if self.stop:
            self.movement_history.append(0)
...
class TrafficLight(ap.Agent):
    ...
    def setup(self):
        self.position1 = (20, 20)  # Posición del primer semáforo
        self.position2 = (20, 20)  # Posición del segundo semáforo
        self.state1 = "red"  # Estado inicial del primer semáforo
        self.state2 = "green"    # Estado inicial del segundo semáforo
...
class IntersectionModel(ap.Model):
    def setup(self):
        self.agents = ap.AgentList(self, 6, Vehicle)  
